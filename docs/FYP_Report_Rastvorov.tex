...
This expansion is reflected in the current project artefacts. The file \texttt{ud\_merged\_treebank\_sentences.report.json} records \texttt{26,416} sentences after merging \texttt{UD\_English-EWT} and \texttt{UD\_English-GUM}; of these, \texttt{14,586} rows came from \texttt{EWT} and \texttt{11,830} from \texttt{GUM}. The \texttt{GUM} layer contributed not only additional sentences, but also genre variation, including \texttt{academic}, \texttt{court}, \texttt{essay}, \texttt{news}, \texttt{speech}, and \texttt{textbook}. The file \texttt{projection\_corpus\_external\_v1.report.json} records \texttt{28,195} final rows assembled from \texttt{ingested\_sentences.jsonl} and the merged UD layer. The distribution by source in this set is as follows: \texttt{13,395} rows from \texttt{ud\_ewt\_treebank}, \texttt{11,827} from \texttt{ud\_gum\_treebank}, \texttt{1,190} from \texttt{tatoeba}, \texttt{1,190} from \texttt{ud\_ewt}, and \texttt{593} from \texttt{wikinews}. In addition, the repository now contains \texttt{simplewiktionary-latest.xml.bz2}, meaning that the Wiktionary layer also entered the project as an actual source asset rather than as a merely planned resource.
