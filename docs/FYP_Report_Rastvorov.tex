...
\begin{enumerate}
  ...
  \item What model of representation best captures the relationships between sentence, phrase, and word nodes while remaining stable enough for validation, persistence, and visualization?
  \item To what extent can structural abstraction, particularly placeholder-based modelling, enhance the generalization of generated pedagogical notes?
\end{enumerate}
...
Collectively, these contributions place the thesis in the position of both an implementation report and an argument about representational design. The software contribution is the operational ELA stack itself. The research contribution is the assertion that stronger pedagogical behaviour emerged when linguistic variation was restructured into more stable structural forms.

\section{Thesis Structure}
The remainder of this report is structured as follows. In Chapter 2 we review the background fields applicable to ELA. Chapter 3 describes the current work as a continuation of the earlier research thesis and explains how the project evolved. Chapter 4 presents the RLE data model. Chapter 5 examines the context--pattern duality. In Chapter 6 we describe the implemented system architecture. Chapter 7 covers dataset and corpus engineering. Chapter 8 evaluates the system. Chapter 9 discusses limitations and future work. Chapter 10 concludes the thesis.

\chapter{Background and Related Work}
\section{Computer-Assisted Language Learning}
Computer-Assisted Language Learning (CALL) provides the pedagogical backdrop for ELA. Classical CALL systems focused on drills, exercises, and tightly constrained curricula [4], while later work shifted toward broader software-mediated support for second-language acquisition [5]. More recent systems incorporate adaptation, learner modelling, multimodal content, and mobile delivery. Even so, a large part of practical CALL still prioritizes task completion and fluency support over structural transparency. The learner receives an answer, a translation, or a corrected sentence, but not necessarily an interpretable account of why the form is what it is.

ELA aligns with a narrower strand of CALL that values learner autonomy, authentic materials, and graduated support. In that tradition, software should not simply replace the learner's own analysis; it should externalize part of that analysis in a form the learner can inspect. This matters especially in independent learning scenarios, where the system must function not only as an exercise engine but as a structured explanatory companion.

\section{Authentic Materials and Cognitive Support}
Authentic materials are attractive because they preserve the lexical, syntactic, pragmatic, and stylistic richness of real communication. This emphasis is well established in language-teaching literature on authenticity and materials design [1]--[3]. Such materials expose the learner to naturally occurring tense shifts, ellipsis, collocation, discourse markers, and speaker intent in ways that sanitized textbook examples often do not. At the same time, the very properties that make authentic input valuable also make it cognitively demanding.

This creates a design tension. If material is simplified too aggressively, authenticity is lost. If it is left untouched without support, the learner can become overloaded. ELA addresses this gap by layering support over the original input rather than rewriting it. The project therefore sits at the intersection of authenticity, scaffolding, and transparency: it aims to preserve the original utterance while exposing interpretable structure around it.

\section{NLP Parsing and Structured Analysis}
The system relies on modern NLP tooling to obtain syntactic and morphological signals, including tokenization, dependency parsing, lemmatization, and part-of-speech tagging. These signals are useful, but they are not themselves a learner-facing representation. Raw parser output is often unstable, implementation-specific, and difficult to expose directly in a pedagogical interface.

ELA therefore treats parsing as an evidence source rather than as the final user model. Parser outputs are normalized into a controlled hierarchical representation that supports enrichment, validation, storage, and visualization. This distinction matters: the project is not merely a wrapper over an NLP library but an attempt to convert low-level linguistic signals into a stable pedagogical contract.

\section{Tense, Aspect, and Mood}
Verb morphology is central to pedagogical explanation in ELA because tense, aspect, and mood are among the most common sources of learner confusion. In many learning tools, TAM interpretation is either hidden inside a black-box model or reduced to surface labels that are hard to audit. ELA adopts a more conservative approach. The implemented system uses a deterministic rules layer over parser output to infer TAM-related information and to maintain traceable links between grammatical evidence and pedagogical phrasing.

This is not only an engineering convenience but also a methodological choice. A deterministic TAM layer makes failure analysis easier, allows targeted tests, and prevents downstream note generation from becoming the sole source of grammatical truth.
...
The repository shows that multiple approaches were explored, including classifier-driven and tabular strategies. The implemented system converges on a lighter-weight deployment path because it integrates more easily into a contract-oriented architecture and is easier to reason about operationally. This reflects a broader pattern in the project: simpler, auditable components are preferred when they fit the explanatory goals better than heavier black-box alternatives.
...
However, the project evidence suggests that representation strategy matters at least as much as model choice. The strongest note-generation behaviour did not come simply from scaling model size or training longer. Instead, it emerged when the training signal was restructured through placeholders and canonicalized patterns. This observation motivates the later discussion of context--pattern duality.
...
\begin{enumerate}
  \item \textbf{Architectural maturation.} Earlier design descriptions emphasized a concept-driven prototype and a more speculative integration of external AI services. The current implementation moves the core truth layer toward deterministic parsing, rules, and validation, with generated content treated as an enrichment path rather than the foundation of correctness.
