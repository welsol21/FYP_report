...
\paragraph{Phase 1: Raw corpora and free-form generation.}
...
\paragraph{Phase 2: Grammar blueprints.}
To stabilize output quality, the project introduced a library of over 150 \emph{grammar blueprints}---hand-written, rule-based note templates keyed to specific grammatical classes such as \texttt{present\_simple\_affirmative}, \texttt{relative\_clause}, or \texttt{passive\_voice}. Each blueprint provided a fixed note at three CEFR tiers (elementary, intermediate, advanced). Blueprints served as a deterministic fallback when model output failed quality gates. The approach improved consistency, but the inverse problem now became clear: the blueprint library itself was too coarse. The same note appeared thousands of times in the projected corpus, and the model had no incentive to learn anything except which blueprint to select.

\paragraph{Phase 3: Slot-based templates.}
...
\paragraph{Phase 4: Grammar books as the source of patterns.}
...
\paragraph{The resulting design.}
Once grammar books became the primary pattern source, the pipeline acquired the following properties. Every template originates from a human expert; the model is never asked to invent explanations. Template diversity is bounded by the breadth of the source library rather than by the engineering team's time. The corpus projection step can expand a book-sourced template to thousands of new sentences while preserving its author-validated wording. And the ``closed'' character is not a restriction but a consequence: because all patterns come from books, all generated notes are traceable to a specific source, author, and pedagogical context.

A deliberate three-tier design governs how note templates are treated at different CEFR levels:

\begin{itemize}
  \item \textbf{Elementary (A1--A2)}: fixed passthrough templates. Notes describe simple, unambiguous rules that do not depend on the specific surface content of the matched sentence. They serve as a \emph{lower-bound baseline} in evaluation.
  \item \textbf{Intermediate (B1--B2)}: slot-templated generation. This is the primary generation task, where the same abstract pattern produces a different, grounded note for each matched sentence via slot filling from the contract.
  \item \textbf{Advanced (C1--C2)}: fixed passthrough templates. Notes describe complex, construction-level rules that are likewise invariant to surface content, acting as an \emph{upper-bound comparison baseline}.
\end{itemize}

By treating the boundary tiers as fixed, the project creates two natural comparison groups against which the quality and diversity of intermediate-level generation can be assessed. Concrete examples from the actual training data are provided in Appendix~\ref{app:pipeline-examples}.
