...
\section{The Closed Pipeline: From Source Book to Templated Dataset}
\label{sec:closed-pipeline}
The dataset construction strategy is deliberately \emph{closed}: every training example originates from a curated pedagogical source book, not from web-scraped text or generic corpora. This design choice is central to the project's educational goals and is what distinguishes ELA's corpus from standard NLP benchmarks.

The closed pipeline proceeds through six concrete stages:

\begin{enumerate}
  \item \textbf{Source extraction.} Grammar handbooks and textbooks (Betty Azar, Mark Lester, Ronald Carter, George Yule, Collins Cobuild, and others) are parsed to produce raw notation--content pairs: a sentence or phrase paired with a handwritten pedagogical note from the original author.
  \item \textbf{Contract construction.} Each content unit is processed by a spaCy-based parser that builds a \emph{linguistic contract} tree. The contract captures the node type (Sentence, Phrase, or Word), part of speech, grammatical role, parent--child links, CEFR prediction, and TAM construction. This structured representation is the canonical form through which all downstream stages operate.
  \item \textbf{Template notation.} The paired pedagogical note is analyzed and converted into a \emph{slot template}: a note string where content-specific terms are replaced by named placeholders drawn from the contract. Allowed slots include \texttt{\{\{HEAD\_NOUN\}\}}, \texttt{\{\{PREPOSITION\}\}}, \texttt{\{\{OBJECT\_NP\}\}}, and \texttt{\{\{RELATIVE\_MARKER\}\}}. Notes that cannot be safely templated remain as passthrough strings.
  \item \textbf{Family alignment.} Templates are grouped into grammar families by template identifier (e.g.\ \texttt{RELATIVE\_CLAUSE\_THAT}, \texttt{PP\_STRANDED}). This alignment step ensures that a note extracted from one source book can be reused when the same grammatical construction appears in a different text.
  \item \textbf{Corpus projection.} The aligned template families are projected onto a larger target corpus by matching contract structures against template identifiers. For every matched instance, the template's slots are filled from the matched contract's attributes, producing a new rendered note grounded in a new sentence. The v17 slot-normalized projection yielded 1,984 sentence-level and 6,584 phrase-level templated rows.
  \item \textbf{T5 dataset row generation.} Each projected triple---input contract JSON, allowed slots, and rendered note target---is serialized into a structured T5 training row. The model learns to produce a slot template from a contract description, not to hallucinate free text.
\end{enumerate}

The practical consequence of this architecture is that the model is never trained to invent grammatical explanations. Instead, it learns to \emph{select and instantiate} patterns that were validated by human authors. The generalization comes not from the model's freedom but from the breadth of the projected corpus. This is what the project documentation calls the \emph{pattern$\to$content} direction: frozen pedagogical patterns, applied to new contexts.

Concrete examples from the actual training data---including slot templates, slot values, and rendered notes---are provided in Appendix~\ref{app:pipeline-examples}.
...
This appendix provides concrete examples drawn from the actual training data generated by the closed pipeline described in Section~\ref{sec:closed-pipeline}. The examples are organized by the two audience tiers currently present in the book-note corpus: \emph{elementary} (corresponding to CEFR A1--A2) and \emph{intermediate} (corresponding to CEFR B1--B2). For each example the table shows the source phrase or sentence, the grammatical topic as labeled in the source book, the note template (with any \texttt{\{\{SLOT\}\}} placeholders shown explicitly), the slot values resolved from the linguistic contract, and the final rendered note that would be displayed to the learner.

\paragraph{Note on C1--C2 coverage.}
The current book-note corpus contains 16 elementary-tier rows and 323 intermediate-tier rows. Advanced-tier notes (C1--C2) are not yet represented in the source book extraction because the pedagogical handbooks processed so far focus primarily on learners at A1--B2. The CEFR prediction classifier, trained on a separate synthetic dataset, does predict all six levels (A1, A2, B1, B2, C1, C2); however, the corresponding \emph{pedagogical note templates} for upper-advanced constructions are a planned extension of the closed pipeline. This gap is an acknowledged limitation of the current dataset scope and is recorded as a next-step item in the project repository.
...
The slot mechanism makes the difference between the two tiers concrete: elementary notes are self-contained pedagogical rules, while intermediate notes are abstract patterns that become specific only when instantiated against a real contract. Both forms are generated by the same closed pipeline; only the template kind---passthrough versus slotted---differs.
