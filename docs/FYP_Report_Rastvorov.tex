...
A deliberate three-tier design governs how note templates are treated at different CEFR levels:

\begin{itemize}
  \item \textbf{Elementary (A1--A2)}: fixed passthrough templates. Notes at this level describe simple, unambiguous rules that do not depend on the specific content of the target sentence. They serve as a \emph{lower-bound baseline} in evaluation.
  \item \textbf{Intermediate (B1--B2)}: slot-templated generation. This is the primary generation task. Note templates contain named placeholders filled from the linguistic contract, so the same abstract pattern produces a different, grounded note for every matched sentence in the corpus.
  \item \textbf{Advanced (C1--C2)}: fixed passthrough templates. Notes at this level describe complex, construction-level rules that are also invariant to surface content. Like the elementary tier, they act as a \emph{comparison baseline} rather than a generation target.
\end{itemize}

This three-tier design is intentional: by treating the boundary tiers as fixed, the project creates two natural comparison groups against which the quality and diversity of intermediate-level generation can be assessed. The generation challenge is thus scoped to exactly the tier where content-specific slot filling adds the most pedagogical value.
...
For each example the table shows the source phrase or sentence, the grammatical topic as labeled in the source book, the note template (with any \texttt{\{\{SLOT\}\}} placeholders shown explicitly for intermediate examples), and the final rendered note that would be displayed to the learner.
